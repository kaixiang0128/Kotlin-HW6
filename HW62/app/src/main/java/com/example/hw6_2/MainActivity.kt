...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_query.setOnClickListener(View.OnClickListener {
            val c: Cursor
            c = if (ed_book.length() < 1) dbrw!!.rawQuery("SELECT * FROM myTable"
                    , null)
            ...
            if (ed_book.length() < 1 || ed_price.length() < 1)
                ...
                try {
                    dbrw!!.execSQL("INSERT INTO myTable(book, price) VALUES(?,?)"
                            , arrayOf<Any>(ed_book.getText().toString(),
        ...
        btn_update.setOnClickListener(View.OnClickListener {
            if (ed_book.length() < 1 || ed_price.length() < 1)
                Toast.makeText(this@MainActivity, "欄位請勿留空",
                    Toast.LENGTH_SHORT).show() else {
                try {
        ...
        btn_delete.setOnClickListener(View.OnClickListener {
            if (ed_book.length() < 1) Toast.makeText(this@MainActivity
                    , "書名請勿留空",
                    Toast.LENGTH_SHORT).show() else {
                try {
